...
#if IS_ENABLED(CONFIG_CUSTOM_WIDGET_BATTERY_STATUS_SHOW_PERCENTAGE)
    char perc[5] = {};
...
#if IS_ENABLED(CONFIG_USB_DEVICE_STACK)
ZMK_SUBSCRIPTION(c_widget_battery_status, zmk_usb_conn_state_changed);
#endif /* IS_ENABLED(CONFIG_USB_DEVICE_STACK) */

int custom_widget_battery_status_init(struct custom_widget_battery_status *widget, lv_obj_t *parent) {
    ...
    c_widget_battery_status_init();
    return 0;
}

lv_obj_t *custom_widget_battery_status_obj(struct custom_widget_battery_status *widget) {
    return widget->obj;
}
